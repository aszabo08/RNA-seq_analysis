...
The plot below compares the log2 fold changes in the two datasets considering the intersection of genes after the initial differential expression analysis. The points that represent the genes can be generally fitted to the f(x) = x line which indicates a correlation between the two datasets.




```{r comp_log2fc, echo=FALSE}
...
Spearman correlation was used to decipher the relationship between the two datasets.



```{r comp_cor, echo=FALSE}
...
The heatmap below shows all the differentially expressed genes in both datasets, comparing their change of magnitude. Their response to the influenza A virus is highly similar, showing almost identical log2 fold changes in the genes.
...
Based on the Venn-diagram below the number of genes that is differentially expressed in both datasets is 1379.
The full list of these genes with annotation is available on here.
